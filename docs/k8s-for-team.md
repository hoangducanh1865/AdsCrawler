...
│   ├── kafka-connect-deployment.yaml   ← Kafka Connect (S3 + JDBC Sink)
│   ├── minio-deployment.yaml
│   ├── spark-master-deployment.yaml
│   ├── spark-worker-deployment.yaml
│   ├── airflow-scheduler-deployment.yaml
│   ├── airflow-webserver-deployment.yaml
│   ├── batch-consumer-deployment.yaml  ← Consumer ghi raw data vào MinIO
│   ├── speed-layer-deployment.yaml     ← Spark Streaming real-time
...
└── jobs/                   ← Tác vụ khởi tạo một lần
    ...
    └── kafka-connect-init-job.yaml  ← Đăng ký S3 Sink + JDBC Sink connector
```
...
```
                    ...
                    │                                                    │
  Raw JSON files    │  ┌──────────┐   ┌──────────────────────────────┐  │
  (mock data)  ─────┼─►│  Kafka   │──►│        Kafka Connect          │  │
                    │  │(broker)  │   │  S3 Sink  ──────────► MinIO   │  │
                    │  └────┬─────┘   │  JDBC Sink ─────► ClickHouse  │  │
                    │       │         └──────────────────────────────┘  │
                    │       │                                            │
                    │  ┌────▼──────────────┐  ┌──────────────────────┐  │
                    │  │   Speed Layer     │  │   Batch Consumer     │  │
                    │  │ (Spark Streaming) │  │  (raw JSON → MinIO)  │  │
                    │  └────┬──────────────┘  └──────────────────────┘  │
                    │       │ processed_* topics                         │
                    │       ▼                                            │
                    │  ┌───────────┐       ┌──────────────────────────┐ │
                    ...
                    │  ┌─────▼──────┐  ┌──────────────────────────────┐ │
                    │  │  Superset  │  │          Airflow             │ │
                    │  │(dashboard) │  │  Webserver (UI) + Scheduler  │ │
                    │  └────────────┘  └──────────────────────────────┘ │
                    │                                                    │
                    │  ┌──────────────┐  ┌──────────────────────────┐   │
...
|---------|---------------------|
| **Kafka** | "Đường ống dữ liệu" — nhận dữ liệu thô từ script mock, đưa vào topic để các consumer xử lý |
| **Kafka Connect** | Tự động hút dữ liệu từ Kafka ra ngoài: S3 Sink ghi vào MinIO, JDBC Sink ghi vào ClickHouse |
| **Batch Consumer** | Đọc raw topics từ Kafka, lưu file JSON thô vào MinIO (datalake) |
| **Speed Layer** | Spark Structured Streaming — xử lý dữ liệu real-time, ghi kết quả ra `processed_*` topics |
| **Spark Master** | Não bộ của cụm Spark, điều phối công việc xử lý từ Speed Layer |
| **Spark Worker** | Tay chân của Spark, thực sự chạy các Spark job được giao từ Master |
| **ClickHouse** | Database phân tích lưu dữ liệu đã xử lý, phục vụ query nhanh cho Superset |
| **MinIO** | Kho lưu trữ file dạng JSON/Parquet (như S3 local) — data lake của dự án |
...
Giai đoạn 2 (chờ Giai đoạn 1 xong):
  ...
  minio-init-job         ← Chờ MinIO healthy → tạo bucket "marketing-datalake"
  Kafka Connect          ← Chờ Kafka sẵn sàng → cài S3/JDBC plugin, khởi động REST API

Giai đoạn 3 (chờ Giai đoạn 2 xong):
  kafka-connect-init-job ← Chờ kafka-connect:8083 → đăng ký S3 Sink + JDBC Sink connector
  Airflow Webserver  ─┐  Có InitContainer chờ Postgres
  Airflow Scheduler  ─┘
  Spark Worker           ← Tự kết nối Spark Master
  Batch Consumer         ← Khởi động, đọc raw topics từ Kafka
  Speed Layer            ← Spark Structured Streaming ghi processed_* topics
